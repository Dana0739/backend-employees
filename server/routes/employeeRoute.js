...
module.exports = function (passport) {
    ...
    router.get('/database/time', asyncHandler(async (request, response) => {
    ...
    router.route('/')
        .get(asyncHandler(async (request, response) => {
            ...
            let employees;
            if (params.error) {
                employees = {status: 400, value: params.error};
            } else {
        ...
        .post(passport.authenticate('jwt', {session: false}, null),
            asyncHandler(async (request, response) => {
                ...
                let employee;
                if (body.error) {
                    employee = {status: 400, value: body.error};
                } else {
